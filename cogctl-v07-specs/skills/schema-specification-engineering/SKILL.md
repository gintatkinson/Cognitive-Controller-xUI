...
## Step 2: Exhaustive Feature Extraction & Constraint Mapping
...
2. **Exhaustive Constraint Parsing:** For EVERY leaf node within the grouped feature, analyze and record all structural constraints:
...
3. **Acceptance Criteria Translation:** Transform these programmatic constraints into exhaustive Given-When-Then Logical Acceptance Criteria that validate the exact data parameters of the nodes.
4. **Draft the Feature Specs:** Write each Feature as a local markdown file (e.g., `docs/features/feat-01-name.md`).
...
4. **Epic Markdown Assembly:**
   ...
   - Ensure the body of the Epic explicitly lists its child features as a Markdown tasklist referencing BOTH the GitHub Issue ID and the local relative markdown path:
     `- [ ] #[IssueID] - [Feature Title](../features/feat-01.md)`
